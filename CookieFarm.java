...
public class CookieFarm {
  private double C;  // # of cookies needed to buy farm
  private double F; // # extra cookies / second
  private double X;  // # of cookies to collect
  private static CookieFarm parse(BufferedReader in) throws IOException {
    CookieFarm cf = new CookieFarm();
    String[] toks = in.readLine().split(" ");
    cf.C = Double.parseDouble(toks[0].trim());
    cf.F = Double.parseDouble(toks[1].trim());
    cf.X = Double.parseDouble(toks[2].trim());
    return cf;
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i=0; i<N; i++) {
      CookieFarm cf = parse(in);
      int max = (int) (cf.X/cf.C)+100;
      if (cf.X < cf.C) {
        System.out.println(String.format("Case #%d: %.7f", i+1, cf.X/2));
      } else {
        Double best = null;
        for (int j=0; j<max; j++) {
          double secs = cf.solve(j);
          //System.out.println(cf + ", farms " + j + " in " + secs + " seconds ");
          if (best == null || best > secs) {
            best = secs;
          }
        }
        System.out.println(String.format("Case #%d: %.7f", i+1, best));
      }
    }
